...
MasonID=aubclsf0051          ## Example: MyID=aubrmg001
MyID=aubclsf0040

WD=/scratch/$MyID/FinalProj                      ## Example:/scratch/$MyID/PracticeRNAseq  
CD=/scratch/$MasonID/FinalProj/CleanData             ## Example:/scratch/$MyID/FinalProj/CleanData   #   *** This is where the cleaned paired files are located from the last script
REFD=/scratch/$MyID/FinalProj/RefGenome    ## Example:/scratch/$MyID/PracticeRNAseq/DaphniaRefGenome    # this directory contains the indexed reference genome for Daphnia
MAPD=/scratch/$MyID/FinalProj/Map_HiSat2           ## Example:/scratch/$MyID/PracticeRNAseq/Map_HiSat2      #
COUNTSD=/scratch/$MyID/FinalProj/Counts_StringTie       ## Example:/scratch/$MyID/PracticeRNAseq/Counts_StringTie
RESULTSD=/home/$MyID/FinalProj/Counts_H_S          ## Example:/home/aubtss/PracticeRNAseq/Counts_H_S

REF=GCF_000002285.5         ## This is what the "easy name" will be for the genome reference
...
ls | grep ".fastq" |cut -d "_" -f 1| sort | uniq > ${MAPD}/list    #should list Example: SRR629651
...
do
  ...
  ##  -p indicates number of processors, --dta reports alignments for StringTie --rf is the read orientation
   hisat2 -p 6 --dta --phred33       \
    -x "${REFD}"/${REF}_index       \
    -1 "${CD}"/"$i"_R1_paired.fastq  -2 "${CD}"/"$i"_R2_paired.fastq      \
    -S "$i".sam
